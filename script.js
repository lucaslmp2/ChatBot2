...
function processarResposta2(resposta) {
  if (resposta.error) {
    ...
    const sortedTimes = resposta.times.sort((a, b) => a.titulos - b.titulos);
    // Display information for each time
    sortedTimes.forEach((time, index) => {
      const message = `${index + 1}. Time: ${time.nome}, Anos dos titúlos: ${time.anos.join(', ')}, Títulos: ${time.titulos}`;
...
function processarResposta(resposta) {
  if (resposta.error) {
    ...
    const message = `Time com mais títulos: ${timeComMaisTitulos.nome}, Anos dos titúlos: ${timeComMaisTitulos.anos.join(', ')}, Títulos: ${timeComMaisTitulos.titulos}`;
    addMessage('Guru', message);
  }
}
...
async function typeMessage(message, element) {
  ...
  const delay = 50; // ajuste esse valor conforme necessário
    ...
    if (!isTyping) {
      ...
      switch (normalizedUserMessage) {
        ...
        case 'titulos':
          ...
          jogadorSolicitado = false;
          voltar = false;
          addMessage('Guru','Digite o ano do campeonato:')
          userInput.value = '';
          break;
        case 'maior campeao':
          todosCampeoes = false;
        ...
        default:
            ...
            if (clubeSolicitado) {
              fetch('clubes.json')
                ...
                .then(data => {
                  ...
                  if (clubeData) {
                    const mensagemClube = `Clube encontrado:
                      Nome: ${clubeData.nome}
                      Abreviação: ${clubeData.abreviacao}
                      Apelido: ${clubeData.apelido}`;
                    ...
                    const mensagemNenhumClube = `Desculpe, não foi possível encontrar um clube com o nome "${nomeClube}". Por favor, tente novamente.`;
              ...
              if (isValidYear) {
                  ...
                  addMessage('Bot', 'Por favor, ano inválido (formatodeve ser: YYYY). Refaça a consulta!');
              }
          }
          else if (maiorCampeao) {
            try {
                const dadosMaiorCampeao = await carregarDadosMaiorCampeao();
          ...
          try {
            const dadosMaiorCampeao = await carregarDadosMaiorCampeao();
            processarResposta2(dadosMaiorCampeao);
        } catch (error) {
            console.error('Erro ao obter o maior campeão:', error.message);
            addMessage('Guru', 'Ops, algo deu errado ao obter o maior campeão.');
        }
